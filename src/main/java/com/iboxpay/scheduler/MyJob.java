...
public class MyJob implements Job {
  ...
  public static void addJobWithCrionTrigger() {
    try {
      ...
      CronTrigger cronTrigger = TriggerBuilder.newTrigger().withIdentity("myJob", "myJob").withSchedule(CronScheduleBuilder.cronSchedule("0/2 * * * * ?")).build();
      // 指定JobDetail和触发器
  ...
  public static void modifyJobTime() {
    try {
      ...
      CronTrigger cronTrigger = TriggerBuilder.newTrigger().withIdentity("myJob", "myJob").withSchedule(CronScheduleBuilder.cronSchedule("0/3 * * * * ?")).build();
  ...
  public static void addJobWithSimpleTrigger() {
    try {
      Scheduler scheduler = factory.getScheduler();
      JobDetail jobDetail = JobBuilder.newJob(MyJob.class).withIdentity("myJob", "myJob").build();
      SimpleTrigger trigger = (SimpleTrigger) TriggerBuilder.newTrigger().withIdentity("myJob", "myJob").startAt(DateBuilder.futureDate(5, IntervalUnit.SECOND)).forJob(jobDetail).build();
